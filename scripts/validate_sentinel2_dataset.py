"""CLI to validate Sentinel-2 samples with PSNR/SSIM and preview outputs."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Sentinel-2 dataset samples with PSNR/SSIM and generate previews."
    )
    ...
    manifest_group.add_argument(
        "--sample",
        action="store_true",
        help="Use the bundled Sentinel-2 sample dataset manifest.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/sentinel2_validation"),
...
def _sample_manifest_path() -> Path:
    return Path(__file__).resolve().parent / "sample_data" / "sentinel2_sample" / "manifest.json"
